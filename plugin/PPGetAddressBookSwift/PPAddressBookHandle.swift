...
class PPAddressBookHandle {
    ...
    private func getDataSourceFrom_IOS9_Later(personModel success: PPPersonModelClosure, authorizationFailure failure: AuthorizationFailure) {
        ...
        let fetchKeys = [CNContactFormatter.descriptorForRequiredKeys(for: CNContactFormatterStyle.fullName),CNContactPhoneNumbersKey,CNContactPostalAddressesKey, CNContactThumbnailImageDataKey] as [Any]
        ...
        for contact in contacts {
            ...
            for labelValue in contact.postalAddresses {
                ...
                model.addressese.append("\(address.country)\(address.state)\(address.city)\(address.street)")
            }

            if model.addressese.count > 0 {
                print(model.name)
            }
            // 将联系人模型回调出去
            success(model)
